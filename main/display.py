...
class Display:
    ...
    def format_text(text):
        ...
        line_break_positions = get_sequence_start_and_end_indexes(
            Display.line_break_sequence)
        double_break_positions = get_sequence_start_and_end_indexes(
            Display.double_line_break_sequence)
        page_break_positions = get_sequence_start_and_end_indexes(
            Display.page_break_sequence)
        ...
        while target_index < final_index:
            ...
            for i in range(target_index, final_index + 1):
                ...
                if (i + 1) % Display.line_length == 0 and i > 0 and i < final_index:
                    ...
                    if line_begin_character == " " and not re.search(empty_line_pattern, text[i + 1: i + Display.line_length + 1]):
                        ...
                        line_break_positions = {
                            int(key) - 1: value - 1 for key, value in line_break_positions.items()}
                        double_break_positions = {
                            int(key) - 1: value - 1 for key, value in double_break_positions.items()}
                        page_break_positions = {
                            int(key) - 1: value - 1 for key, value in page_break_positions.items()}
                ...
                if i in line_break_positions or i in double_break_positions or i in page_break_positions:
                    ...
                    spaces_added = 0
                    spaces_from_end_of_line = Display.line_length - \
                        (i % Display.line_length)

                    if i in line_break_positions:

                        text = text[:i] + f"{' ' * spaces_from_end_of_line}" + \
                            text[line_break_positions[i]:]

                        del line_break_positions[i]

                        spaces_added = spaces_from_end_of_line - \
                            len(Display.line_break_sequence)

                    elif i in double_break_positions:

                        text = text[:i] + f"{' ' * (spaces_from_end_of_line + Display.line_length)}" + \
                            text[double_break_positions[i]:]

                        del double_break_positions[i]

                        spaces_added = spaces_from_end_of_line + Display.line_length - \
                            len(Display.double_line_break_sequence)

                    elif i in page_break_positions:

                        line_number = (i // Display.line_length)
                        empty_lines_required = Display.printable_lines - \
                            (line_number % Display.printable_lines) - 1

                        text = text[:i] + f"{' ' * ((spaces_from_end_of_line + (Display.line_length * empty_lines_required)))}" + \
                            text[page_break_positions[i]:]

                        del page_break_positions[i]

                        spaces_added = (spaces_from_end_of_line + (Display.line_length *
                                                                   empty_lines_required)) - len(Display.line_break_sequence)

                    # Update index dictionaries to reflect added spaces
                    line_break_positions = {int(
                        key) + spaces_added: value + spaces_added for key, value in line_break_positions.items()}
                    double_break_positions = {int(
                        key) + spaces_added: value + spaces_added for key, value in double_break_positions.items()}
                    page_break_positions = {int(
                        key) + spaces_added: value + spaces_added for key, value in page_break_positions.items()}
                ...
                if i not in ignore_overflow_indexes and (i + 1) % Display.line_length == 0 and i > 0 and i < final_index:
                    ...
                    if line_ending_character != " " and line_begin_character != " ":
                        ...
                        if spaces_to_add < Display.line_length:
                            ...
                            text = text[:i - spaces_to_add] + \
                                f"{' ' * spaces_to_add}" + \
                                text[i - spaces_to_add:]
    ...
    def text_to_lines(text):
        ...
        for i in range(total_lines):
            ...
            if i < total_lines - 1:
                result.append(
                    text[i * Display.line_length: (i + 1) * Display.line_length])
            else:
    ...
    def centre_justify_lines(text_lines_list):
        ...
        if not isinstance(text_lines_list, list):
            raise TypeError(
                "Display.centre_justify_lines(): Input must be a list")

        for item in text_lines_list:
            if not isinstance(item, str):
                raise TypeError(
                    "Display.centre_justify_lines(): All elements in the list must be strings")
    ...
    def lines_to_pages(text_lines_list):
        ...
        for i in range(total_pages):
            ...
            if i < total_pages - 1:
                result.append(
                    text_lines_list[i * Display.printable_lines: (i + 1) * Display.printable_lines])
    ...
    def remove_empty_lines_in_text(text):
        ...
        temp_lines = [text[i:i+Display.line_length]
                      for i in range(0, len(text), Display.line_length)]

        empty_line_pattern = r"\s{" + str(Display.line_length) + "}"
        temp_lines_without_empty = [
            line for line in temp_lines if not re.search(empty_line_pattern, line)]
    ...
    def format_multiple_texts_in_list_to_lines(text_list):
        ...
        formatted_items_empty_lines_removed = [
            Display.remove_empty_lines_in_text(entry) for entry in formatted_items]

        formatted_items_trailing_whitespace_removed = [
            entry.rstrip() for entry in formatted_items_empty_lines_removed]

        items_to_lines = [Display.centre_justify_lines(Display.text_to_lines(
            text)) for text in formatted_items_trailing_whitespace_removed]
        ...
        for i, item in enumerate(items_to_lines):
            ...
            if i < len(items_to_lines) - 1:
                items_with_seperating_newlines.append(
                    [f"{' ' * Display.line_length}"])

        flattened_lines_with_bumpers = [
            item for sublist in items_with_seperating_newlines for item in sublist]
        ...
        for key, value in dict.items():
            ...
            if len(value) + len(key) + len(dividing_string) > Display.line_length:
                ...
                if len(key) + len(dividing_string) > Display.line_length // 2 and len(value) > Display.line_length // 2:
                    key = key[:(Display.line_length // 2) -
                              len(truncate_dividing_string)] + truncate_dividing_string
                    value = value[:(Display.line_length // 2) -
                                  len(truncate_string)] + truncate_string

                # if key is over half a line's length
                elif len(key) + len(dividing_string) > Display.line_length // 2:
                    key = key[:Display.line_length -
                              len(truncate_dividing_string) - len(value)] + truncate_dividing_string

                # if value is over half a lines length
                elif len(value) > Display.line_length // 2:
                    key = key + dividing_string
                    value = value[:Display.line_length -
                                  len(truncate_string) - len(key)] + truncate_string
        ...
        for i, (key, value) in enumerate(dict.items()):
            ...
            if len(value) + len(key) + len(dividing_string) > maximum_item_length:
                ...
                if len(key) + len(dividing_string) > maximum_item_length // 2 and len(value) > maximum_item_length // 2:
                    key = key[:(maximum_item_length // 2) -
                              len(truncate_dividing_string)] + truncate_dividing_string
                    value = value[:(maximum_item_length // 2) -
                                  len(truncate_string)] + truncate_string

                # if key is over half a line's length
                elif len(key) + len(dividing_string) > maximum_item_length // 2:
                    key = key[:maximum_item_length -
                              len(truncate_dividing_string) - len(value)] + truncate_dividing_string

                # if value is over half a lines length
                elif len(value) > maximum_item_length // 2:
                    key = key + dividing_string
                    value = value[:maximum_item_length -
                                  len(truncate_string) - len(key)] + truncate_string
    ...
    @staticmethod
    def test3(text_list):
        formatted_texts = Display.format_multiple_texts_in_list_to_lines(
            text_list)
...
dummy_page = ['  amet, consectetur adipiscing elit. Nam tincidunt egestas  ', 'augue, nec eleifend elit porttitor vitae. Mauris euismod, ex', 'a semper vestibulum, lectus dui odales est, quis solliitudin', '  lectus purus eget sem. In quam diam, porta eu diam vel,   ', 'rhoncus gestas ligula. Nulla pretium quam quis ex fermentum ', '   condimentum. Ut nec mi tempor est consequat accumsan.    ', 'Integer cursus nisl enim, sit amet laoreet leo tincidunt id.', '  Phasellus et turpis quis u/rna varius molis. Nulla erat   ',
              ' felis lacinia nec leo a, ornare luctus nibh. Pellentesque  ', ' lacus magna, malesuada at imperdiet id, vehicula sit amet  ', '   assa. Ut rhocus, felis vel tempus volutpat, leo ipsum    ', '    aliquam ipsu, nec eleifend velit odio eu purus. Duis    ', 'accumsan purus in pharetra dapibus. Maecenas mollis lobortis', 'vestibulum. Nulla et mauris quis augue tincidunt viverra ut ', 'condimentum lectus. Praesent fringilla diam nunc, id egestas', ' rna venenatis #non.Phasellus id orci scelerisque, ultrices ']

multi_texts = ["tHIS IS THE first line", "this is the second line, it goes on for much longer for some reason so that you can see it overflowing later on lalalalalala la la lal la la lal ", "this is the third line, it contains a /n line break",
               "this is the fourth line, it contains a /d double break and a trailing line break/n", "this is the fifth line, it has a /p page break in, sorry bozo!gsdgdsgdsgsdgdsgsdgdsgdsg  gsdgds  sdg dsg sdgs"]
